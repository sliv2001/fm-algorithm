...
std::map<int, std::list<int>>& GainContainer::getMap(int index){
	return partition.at(index)?right1:left0;
}

void GainContainer::fillContainer() {
	vector<int> gains(reader.getVertexCount(), 0);
	int localGain=0;
	bool isOnlyVertex=1, isOnOneSide=1;
	for (auto& net : reader.getGraphContents()){
		for (auto& vertex : net){
...
std::vector<bool>& GainContainer::getExist(int edgeIndex) {
	return edgeIndex>=0?existPos:existNeg;
}

void GainContainer::iterate() {
	for (int i=0; i<reader.getVertexCount(); i++){
